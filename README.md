...
- В карточке клиента добавлен автопоиск:
  ...
  - для ИП название приводится к формату `ИП Фамилия Имя Отчество`;
  - по БИК (ЦБ) с автозаполнением названия банка и корр. счета.
- На странице товаров добавлены:
  - автоподсказки владельца (по клиентам из базы) и производителя (по всем названиям поставщика: рус/англ/кит);
  - автоформат ТНВЭД при вводе;
  - нормализация десятичных чисел (запятая, автодобавление `0,`);
  - импорт/экспорт Excel: «Загрузить файл», «Скачать шаблон», «Скачать все в Excel» (по текущему фильтру, со всеми атрибутами товара);
  - кнопка «Скопировать товар» (копирует все атрибуты, к артикулу добавляет `(1)`, `(2)` и т.д.).
